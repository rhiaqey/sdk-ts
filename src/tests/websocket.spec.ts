...
describe('websocket', () => {
    it('normalize_endpoint works on dev', () => {
        ...
        expect(tt.testWebsocketNormalization(to)).toEqual(
            'ws://localhost:3002/ws?api_key=some_strong_api_key1&api_host=localhost:5000&channels=ticker3,iss,iss2,ticks,yahoo1&snapshot=true',
        );
    });
    ...
    it('endpoint is generated correctly', () => {
        ...
        expect(tt.getWebSocketEndpoint()).toEqual(
            'wss://localhost:3002/ws?api_key=some_strong_api_key1&api_host=localhost:5000&channels=ticker3,iss,iss2,ticks,yahoo1&snapshot=true',
        );
    });
    ...
    it('snapshot is generated correctly when snapshot is false', () => {
        ...
        const tt = new TestWebsocket(new_options);
        expect(tt.getWebSocketEndpoint()).toEqual(
            'wss://localhost:3002/ws?api_key=some_strong_api_key1&api_host=localhost:5000&channels=ticker3,iss,iss2,ticks,yahoo1&snapshot=false',
        );
    });
